...
class EstateProperty(models.Model):
    ...
    state = fields.Selection(selection=[('new','New'),('recieved','Offer Recieved'),('accept','Offer Accepted'),('reject','Offer Rejected'),('sold','Sold'),('cancel','Canceled')],default="new",copy=False) 
    ...
    def sold_button(self):
        # print('>>>>>>>>>>>>>>>>>>>>>>>>>>',(self.expected_price*90)/100)
        # print('>>>>>>>>>>>>>>>>>',self.offer_ids.state)
        for rec in self:
            for orec in rec.offer_ids:
                if rec.state != "cancel":
                    if orec.state == "accept":
                        print('>>>>>>>>>>>>',orec.state)
                        rec.state = "sold"
                    else:
    ...
    def unlink(self):
        for rec in self:
            if self.state == 'new' or self.state=='cancel':
                return super().unlink()
            else:
                raise UserError(_('You can not delete unless its new or cancled property.'))
